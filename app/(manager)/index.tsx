...
export default function ManagerDashboard() {
  ...
  const loadData = useCallback(async () => {
    const { data: jobList } = await supabase.from('jobs').select('*').order('name');
    if (!jobList) return;
            ...
            <View style={styles.cardHeader}>
              ...
              <View style={styles.badges}>
                ...
                {item.pendingSupplies > 0 && (
                  <View style={[styles.badge, { backgroundColor: '#2a1a0a' }]}>
                    <Text style={[styles.badgeText, { color: '#f97316' }]}>📦 {item.pendingSupplies}</Text>
                  </View>
                )}
              </View>
            </View>
